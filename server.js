...
app.get('/api/:city/:category', function(req, res){
  ...
  if(req.query.min_price == undefined) {
    newData = itemsData.filter((item) => {
       return item.city == req.params.city && item.category == req.params.category && item.price >= request.query.min_price && item.price <= request.query.max_price
     })
  } else {
   newData = itemsData.filter((item) => {
...
// show all the items for tht listing
